...
namespace PhanMemQLTV
{
    public partial class frmBaoCaoThongKe : Form
    {
        ...
        {
            rpt_dssqh rpt = new rpt_dssqh();
            DataTable rptData = new DataTable();
            string strTimSLSQuaHan = "select MaPhieu, MaDG, MaSach, NgayMuon, NgayTra, GhiChu from tblHsPhieuMuon where Convert (datetime, NgayTra, 103)< getdate ()";
            myDataAdapter = new SqlDataAdapter(strTimSLSQuaHan,myConnection);
            myDataAdapter.Fill(rptData);
            rpt.SetDataSource(rptData);

            FrmprvDSSQH f = new FrmprvDSSQH(rpt);
            f.Show();
